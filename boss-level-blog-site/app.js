...
app.get("/posts/:postName", function (req, res) {
  ...
  let reqPostTitle = req.params.postName;
  reqPostTitle = _.lowerCase(reqPostTitle);
  for (let i = 0; i < posts.length; i++) {
    let storedTitle = posts[i].title;
    storedTitle = _.lowerCase(storedTitle);
    console.log("storedTitle:" + storedTitle);
    if (storedTitle === reqPostTitle) {
      res.render("post", { postTitle: posts[i].title, postCont: posts[i].content });
    }
    else {
...
app.get("/post", function (req, res) {
  res.render("post", { postTitle: posts[0].title, postCont: posts[0].content });
})
